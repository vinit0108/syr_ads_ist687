...
doMySentiment <- function(posWords, negWords, stringToAnalyze ) {
  words <- strsplit("This book is horrible", " ")
  count_poswords <- 0
  count_negwords <- 0
  for(word in unlist(words)){
    if(is.element(word, posWords)){
      print(word)
      count_poswords <- count_poswords + 1
    }
    if(is.element(word, negWords)){
      count_negwords <- count_negwords + 1
    }
  }
  sentimentScore <- count_poswords - count_negwords
